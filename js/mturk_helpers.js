           ...
           if ($('.question_section:visible input[type=radio]').length > 0 && $('.question_section:visible input[type=radio]:checked').length <= 0) {
               ...
               continueButton(function() {
                     ...
                     if ($("#platform").val() === 'mturk') {
                        ...
                        // While unchecked checkboxes are contained in formElement, they are not included in the
                        // keys and values generated by FormData (in alignment with WC3 standards). This can lead
                        // to issues with Proliferate (incl, it seems, on subsequent participants) --- perhaps
                        // because we're suddenly handing a different JSON data structure to Proliferate. Thus,
                        // we are creating at least one filled value "NO RESPONSE PROVIDED" for any group of
                        // checkboxes without any checked options.
                        var checkbox = $("#mturk_form").find("input[type=checkbox]");
                        var checkbox_checked = [];
                        // For each name of checkbox collect whether there is at least one checked value
                        for (i = 0; i < checkbox.length; i++) {
                          if (checkbox_checked[checkbox[i].name] === undefined) {
                            if (checkbox[i].checked) {
                              checkbox_checked[checkbox[i].name] = i;
                            } else {
                              checkbox_checked[checkbox[i].name] = -i;
                            }
                        ...
                        // For any checkbox names for which no value has been found (and checkbox_check thus has
                        // a negative value), write a NULL value into that checkbox.
                        for (k in checkbox_checked) {
                          if (checkbox_checked[k] < 0) {
                            throwMessage('Checkbox group ' + checkbox[-checkbox_checked[k]].name + ' has nothing checked. Filling with NO REPONSE PROVIDED as value for compatibility with Proliferate.');
                            checkbox[-checkbox_checked[k]].value = 'NO RESPONSE PROVIDED';
                            checkbox[-checkbox_checked[k]].checked = true;
                          }
                        }
